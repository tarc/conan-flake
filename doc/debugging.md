...
Passing `--trace-verbose` to Nix commands causes conan-flake to print verbose logging of its activity. To enable it:
...
Below there is a sample output when building [haskell-multi-nix](https://github.com/tarc/haskell-multi-nix/tree/debug) with `--trace-verbose`:
