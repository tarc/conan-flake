...
let
  ...
  docs = inputs.conan-flake.lib.packages.docs pkgs;
...
in
{
  options.primaryAgent = lib.mkOption {
    type = lib.types.enum (
      [ "general-purpose" ]
      ++ builtins.attrNames {
        inherit (claudeAgents) supervise;
      }
    );
    default = "general-purpose";
    description = "Which Claude Code agent drives this dev shell by default.";
  };
  ...
  config = {
    ...
    # `docs/node_modules` so that the Astro development server — and anything
    # else run from `docs/` — finds it without a `pnpm install` step of its own.
    # authoring.PREVIEW.3
    #
    # It is copied rather than symbolically linked, which would be the obvious
    # thing to do with a store path: Astro resolves a module identifier through
    ...
    # activation.
    #
    # NOTE: keep this identical to the same hook in `dev/flake.nix`. The
    # destination is reached through `$DEVENV_ROOT`, resolved at activation
    # time: an absolute checkout path baked into the store would make a
    # worktree, a copy or a CI checkout write into the *primary* checkout's
    # `docs/` instead.
    enterShell = ''
      docsNodeModules=${docs.nodeModules}
