{ pkgs, ... }:
{
  ...
  programs = {
    ...
    # [ "README.md" "docs/src/*.md" ]` below for its duration — the same list
    ...
    #
    # Everything above holds for the documentation site's sources as well: they
    # carry the same kind of block, generated by running the same example
    # projects.
    #
  ...
  settings.formatter = {
    ...
    mdsh = {
      # `treefmt-nix` points `mdsh` at `README.md` alone; the site's sources
      # carry command-output blocks too. `mdsh` runs each block from the
      # directory of the file carrying it, which is why the site's blocks `cd`
